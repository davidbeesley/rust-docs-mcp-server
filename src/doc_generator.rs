...
pub fn generate_docs_for_deps(
    ...
    package.insert("name".to_string(), toml::Value::String("docs_only".to_string()));
    package.insert("version".to_string(), toml::Value::String("0.1.0".to_string()));
    package.insert("edition".to_string(), toml::Value::String("2021".to_string()));
    ...
    fs::write(src_dir.join("lib.rs"), "// Empty library for documentation generation")
...
fn run_cargo_doc(dir: &Path, features: &Option<Vec<String>>) -> Result<(), ServerError> {
    ...
    cmd.current_dir(dir)
        .arg("doc")
        .arg("--no-deps"); // Only document the dependencies, not their dependencies
