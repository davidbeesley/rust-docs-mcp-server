...
use std::{collections::HashMap, fs::{self, File, create_dir_all}, io::Write, path::{Path, PathBuf}}; // Added Path, PathBuf, HashMap
...
pub fn load_documents_from_cargo_doc(crate_name: &str) -> Result<Vec<Document>, DocLoaderError> {
    ...
    // Collect all HTML files in the crate documentation directory
    ...
    // Add the crate's root index.html if it exists
    ...
    // Process the filtered list of files
    for path in paths_to_process {
        // Calculate path relative to the crate_doc_path
        let relative_path = match path.strip_prefix(&crate_doc_path) {
            Ok(p) => p.to_path_buf(),
            Err(e) => {
                eprintln!("[WARN] Failed to strip prefix {} from {}: {}", 
                    crate_doc_path.display(), path.display(), e);
                continue;
            }
        };
        let path_str = relative_path.to_string_lossy().to_string();
